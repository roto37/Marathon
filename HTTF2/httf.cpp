...
std::mt19937 mt(rnd());
std::uniform_int_distribution<int> rd1(0, 5), rd2(0, 5), rd4(0, 10000);
//rd1(mt);
...
vector<char> effectLR2 = {'L', 'R'};
vector<char> effectLRB = {'L', 'R', '#'};
vector<char> effectL = {'.', 'L'};
vector<char> effectR = {'.', 'R'};
vector<char> effectDT = {'.', 'D', 'T'};
vector<char> dir = {'N', 'E', 'S', 'W'}; //0:北, 1:東, 2:南, 3:西

inline pair<int, int> next_pos(int d, int x, int y, int e, vector<vector<char>>& g) {
  int nx = x;
  int ny = y;
  if (dir[d] == 'N') {
    if (e == '.') {
...
inline int calc_score(vector<vector<char>>& g) {
...
vector<vector<char>> make_g() {
  ...
  rep2(i, 2, m - 2)rep2(j, 2, m - 2) {
    ret[i][j] = effectLR[Xor128() % 3];
    //ret[i][j] = effectDLR[Xor128() % 4];
    //ret[i][j] = '.';
    //ret[i][j] = effectall[Xor128()%6];
    //ret[i][j] = effectL[Xor128() % 2];
  }
/*
  rep(i, 1000) {
    int x = Xor128() % 28;
...
void solve() {
  ...
  while (check_time()) {
    ...
    */
    ...
    //if (abs(x - 14) + abs(y -14) > 20) continue;
    tmpans[y][x] = effectLR[Xor128()%3];
    //char tmpans = ans[y][x];
    int tmp_score = calc_score(tmpans);

    //double temp        = startTemp + (endTemp - startTemp) * t / T;
    //double probability = exp((tmp_score - max_score) / temp);
    //bool update    = probability > (double)(rd4(mt) / 10000.0);
    //cout << cnt << ":" << tmp_score << endl;
    //cout << cnt << ":" << tmp_score  - max_score << " " << probability <<  endl;
